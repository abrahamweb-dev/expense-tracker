  ...
  filtered.forEach(expense => {
    ...
    item.innerHTML = `
      <div class="expense-left">
        <div class="expense-icon" style="background:${meta.color}22; color:${meta.color};">${meta.icon}</div>
...
function updateBreakdown(list) {
  ...
  const colors = labels.map(l => getMeta(l).color);
  ...
  labels.forEach((label, i) => {
    ...
    row.innerHTML = `
      <div class="legend-left">
        ...
        <span>${getMeta(label).icon} ${label}</span>
      </div>
      <span class="legend-amount">$${values[i].toFixed(2)}</span>
    `;
